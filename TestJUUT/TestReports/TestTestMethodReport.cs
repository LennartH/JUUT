...
namespace TestJUUT.TestReports {
    ...
    public class TestTestMethodReport {
        ...
        public void Creation() {
            ...
            report = new MethodReport(typeof(TestOwner).GetMethod("SetUp"), raisedException);
        ...
        public void ReportToString() {
            ...
            Report report = new MethodReport(typeof(TestOwner).GetMethod("TestName"), raisedException);
            AssertEx.That(report.ToString(), Is.EqualTo(report.Text));
        }

        [TestMethod]
        public void EqualsAndHashCode() {
            Report report = new MethodReport(typeof(TestOwner).GetMethod("TestName"), new AssertException("Exception test."));
            ...
            Report equal = new MethodReport(typeof(TestOwner).GetMethod("TestName"), new AssertException("Exception test."));
            ...
            unequal = new MethodReport(typeof(TestOwner).GetMethod("UnequalMethod"), new AssertException("Exception test."));
            Assert.IsFalse(report.Equals(unequal), "An object shouldn't be equal to an object of an other type.");
            AssertEx.That(report.GetHashCode(), Is.Not(unequal.GetHashCode()), "Unequal objects shouldn't have equal hashcodes.");

            unequal = new MethodReport(typeof(TestOwner).GetMethod("TestName"), new AssertException("Unequal exception test."));
